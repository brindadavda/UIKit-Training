...
class RegisterVC: UIViewController {
    ...
    @IBOutlet weak var phoneNumTF: DesignableUITextField!
    ...
    override func viewDidLoad() {
        ...
        for (key, value) in dialingCodes {
            if let country = countryName(countryCode: key) {
                ...
                arrOfCountriesWithTheirValues.append("\(emojiFlag) \(key) \(value)")
            }
        }
    }
    ...
    @IBAction func sendPhoneNum(_ sender: UIButton) {
        if let countryCode = countryCode.text , let phoneNo  = phoneNumTF.text {
            Task{
                let user = User(username: "username" , phoneNumber: countryCode+phoneNo)
                if await FirebaseDB.shared.addData(user: user){
                    DisplayAlert(AlertType: Alert.Success.rawValue, message: "Phonenumber Added", target: self)
                }else{
    ...
    var arrOfCountriesWithTheirValues = [String]()
