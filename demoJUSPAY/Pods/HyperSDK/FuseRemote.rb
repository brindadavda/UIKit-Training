...
puts ("[HyperSDK] Environment - " + env).yellow if env != "production"
puts ("[HyperSDK] Scope - " + scope).yellow if scope != "release"
...
def should_add_verify_assets_file()
    ...
    iphone_files_exist = required_files.all? do |file|
        File.exist?(File.join($hyper_sdk_iphone_framework_path, file))
    end

    simulator_files_exist = required_files.all? do |file|
        File.exist?(File.join($hyper_sdk_simulator_framework_path, file))
    end

    return iphone_files_exist && simulator_files_exist
end

def add_verify_assets_file()
    if should_add_verify_assets_file()
...
if (merchant_config_data["assetVersion"])
    assetVersion = merchant_config_data["assetVersion"]
    asset_zip_url = "https://assets.juspay.in/hyper-sdk/in/juspay/merchants/hyper.assets." + merchant_name + "/" + assetVersion + "/hyper.assets." + merchant_name + "-" + assetVersion + ".zip"
end
...
if clean_assets || response.code == '200'
    ...
    FileUtils.rm_rf(Dir[ assets_path + "/*.mp3" ])
    FileUtils.rm_rf(Dir[ assets_path + "/app-*.json" ])
...
if response.code == '200'
    ...
    if !add_verify_assets_file()
        puts "[HyperSDK] Error - Required files are missing!".red
        return
    end
